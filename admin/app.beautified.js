import {
   ADMIN_PASSWORD,  STORAGE_KEY,  SESSION_KEY,  GITHUB_TOKEN_KEY,  GITHUB_CONFIG,  API_ENDPOINT,  POSTS_FILE,  MEDIA_FILE 
} from './config.js';
import {
   el 
} from './dom.js';
(function () {
         ...
         let previewState = {
     chapter:  '',  pages:  [],  index:  0 
  };
         let pendingMediaSelection = null;
          // Image upload state       let selectedFiles = [];
         let uploadQueue = [];
          // Page reordering state       let currentPages = [];
         let hasUnsavedChanges = false;
         let draggingIndex = null;
          // ==================== AUTHENTICATION ====================        async function checkSession() {
             ...
             try {
                 ...
                 if (response.ok) {
                     ...
                     if (data.chapters && typeof data.chapters === 'object') {
                         ...
                         chapterFolders = data.chapterFolders || {
            
          };
                         statusMessage = data.statusMessage || '';
                         // Infer missing folder mappings from existing pages               Object.keys(chapters).forEach(name => {
                             if (!chapterFolders[name]) {
                                 const inferred = inferFolderFromPages(name);
                                 if (inferred) {
                         ...
                         console.log(' Loaded chapter data from data.json');
                         return;
    ...
    }          // Fallback to localStorage (saved drafts)         const saved = localStorage.getItem(STORAGE_KEY);
             if (saved) {
                 try {
                     const parsed = JSON.parse(saved);
                     if (parsed && parsed.chapters) {
                         chapters = parsed.chapters;
                         chapterFolders = parsed.chapterFolders || {
        ...
        }             console.log(' Loaded chapter data from localStorage');
                     return;
    ...
    }          // Final fallback:  empty chapters (user can add new ones)         console.warn('No chapter data found, starting with empty chapters');
             chapters = {
      
    };
           
  }        // ==================== PERSISTENCE HELPERS ====================        async function saveToServer(filename,  content) {
         ...
         filename,  content 
              ...
              if (!response.ok) {
                 ...
                 try {
                     ...
                     console.error('Error reading save response',  error);
         ...
         chapters,  chapterFolders,  statusMessage 
      }));
                 ...
                 chapters,            chapterFolders,            statusMessage,            lastUpdated:  new Date().toISOString(),            publishedBy:  'Admin Panel'         
    };
              try {
  ...
  }        // ==================== UI RENDERING ====================        async function showDashboard() {
             ...
             chapterNames.forEach((name,  index) => {
                 ...
                 item.innerHTML = `         <div class="chapter-info">           <div class="chapter-name">${escapeHtml(name)}</div>           <div class="chapter-meta">${pages.length} pages</div>         </div>         <div class="chapter-actions">           <button class="btn-small btn-edit" data-chapter="${escapeHtml(name)}">Edit</button>           <button class="btn-small btn-delete" data-chapter="${escapeHtml(name)}">Delete</button>         </div>       `;
                 el.chapterList.appendChild(item);
               
    });
              // Attach event listeners         document.querySelectorAll('.btn-edit').forEach(btn => {
             ...
             if (el.statusMessageInput) {
                 el.statusMessageInput.value = statusMessage || '';
             ...
             statusMessage = (el.statusMessageInput?.value || '').trim();
             try {
             ...
             currentPages = [...pages];
     // Keep a copy for reordering         el.pageList.innerHTML = '';
              currentPages.forEach((path,  index) => {
                 const item = document.createElement('div');
                 item.className = 'page-item';
                 item.draggable = true;
                 item.dataset.index = index;
                  // Drag event handlers           item.addEventListener('dragstart',  (e) => {
                     draggingIndex = index;
                  ...
                  item.addEventListener('drop',  (e) => {
                     ...
                     item.classList.remove('drag-over');
                      if (draggingIndex !== null && draggingIndex !== index) {
                         // Perform the move               const newPages = [...currentPages];
                         const [moved] = newPages.splice(draggingIndex,  1);
                  ...
                  item.innerHTML = `         <span class="page-number">#${index + 1}</span>         <span class="page-path">${escapeHtml(path)}</span>         <div class="page-actions">           <button type="button" class="btn-move btn-move-up" data-index="${index}" title="Move up" ${index === 0 ? 'disabled' : ''}>UP</button>           <button type="button" class="btn-move btn-move-down" data-index="${index}" title="Move down" ${index === currentPages.length - 1 ? 'disabled' : ''}>DOWN</button>           <button type="button" class="btn-remove" data-index="${index}" data-path="${escapeHtml(path)}">Remove</button>         </div>       `;
                 el.pageList.appendChild(item);
               
    });
              // Attach event listeners for buttons         document.querySelectorAll('.btn-move-up').forEach(btn => {
              ...
              document.querySelectorAll('.btn-remove').forEach(btn => {
                 btn.addEventListener('click',  (e) => {
                     e.stopPropagation();
                     removePage(parseInt(btn.dataset.index),  btn.dataset.path);
  ...
  }        // ==================== CHAPTER OPERATIONS ====================        async function reconcileChapterPages(chapterName) {
             const stored = normalizePages(chapters[chapterName] || []);
    ...
    }          // Use currentPages which contains the reordered pages         const pages = [...currentPages];
             const chapterFolder = ensureChapterFolder(newName);
              // Ensure folder exists on disk for this chapter         try {
           ...
           chapterFolder 
                 ...
                 showError(`Could not create folder for ${newName}: ${e.message}`);
                 return;
               
    }          // Delete old chapter if name changed (and move folder mapping)         if (currentEditingChapter && currentEditingChapter !== newName) {
                 delete chapters[currentEditingChapter];
                 if (chapterFolders[currentEditingChapter]) {
                     chapterFolders[newName] = chapterFolders[currentEditingChapter];
                     delete chapterFolders[currentEditingChapter];
                   
      }         
    }          // Save new/updated chapter         chapters[newName] = pages;
             await saveChapters();
             renderChapterList();
             clearUnsaved();
             hideModal();
             ...
             if (path) {
                 ...
                 currentPages = sortPagesByFilename(currentPages);
                 renderPageList(currentPages);
                 markUnsaved();
               
    }       
  }        async function removePage(index,  imagePath) {
             const targetPath = imagePath || currentPages[index];
  ...
  }        // ==================== BLOG / FEED MANAGEMENT ====================        function getPostsUrl() {
         ...
         cache:  'no-cache' 
                 ...
                 posts = Array.isArray(data)             ? data.map(p => ({
                       ...p,                share:  p.share !== false,                imageTags:  Array.isArray(p.imageTags) ? p.imageTags :  parseTags(p.imageTags || '')             
      }))             :  [];
                 renderPosts();
                 ...
                 posts = [];
                 renderPosts();
                 setPostStatus('Could not load existing posts. Create a new one to get started.',  true);
             ...
             el.postList.innerHTML = '';
              ...
              sorted.forEach(post => {
                 ...
                 const shareLabel = post.share === false ? '<span style="color: var(--danger); font-size: 0.85rem;">Not broadcasting</span>' :  '<span style="color: var(--success); font-size: 0.85rem;">Broadcasting</span>';
                 const tagText = post.imageTags && post.imageTags.length ? `Tags: ${post.imageTags.join(', ')}` :  '';
                 item.innerHTML = `         <div class="chapter-info">           <div class="chapter-name">${escapeHtml(post.title || 'Untitled')}</div>           <div class="chapter-meta">${dateLabel} - ${shareLabel}</div>           ${tagText ? `<div class="chapter-meta" style="opacity:0.8;">${
        escapeHtml(tagText)
      }</div>` : ''}           <div class="chapter-meta" style="opacity:0.8;">${escapeHtml(preview)}</div>         </div>         <div class="chapter-actions">           <button class="btn-small btn-edit" data-post="${post.id}">Edit</button>           <button class="btn-small btn-delete" data-post="${post.id}">Delete</button>         </div>       `;
       ...
       dateStyle:  'medium',  timeStyle:  'short' 
             ...
             try {
                 await saveToServer(POSTS_FILE,  posts);
                 setPostStatus('Post saved and feed updated.');
               
    } catch (error) {
                 console.error('Failed to save posts:',  error);
                 setPostStatus('Failed to save posts to disk. Copy your text before refreshing.',  true);
    ...
    }          // If a file is attached,  upload it and add to media library         if (uploadFile) {
                 try {
                     setPostStatus('Uploading image...',  false);
                     const payload = {
                         name:  uploadFile.name,                data:  await readFileAsBase64(uploadFile)             
        };
             ...
             file:  payload 
              ...
              if (editingPostId) {
                 ...
                 if (idx !== -1) {
                     posts[idx] = {
           ...posts[idx],  title,  image,  imageTags,  content,  date:  now,  share 
        };
                   
      }         
    } else {
                 posts.unshift({
                     id:  generatePostId(),              title,              image,              imageTags,              content,              date:  now,              share           
      });
             ...
             if (el.postImageTags) el.postImageTags.value = (post.imageTags || []).join(', ');
       ...
       behavior:  'smooth',  block:  'start' 
             ...
             if (el.previewSection) {
                 ...
                 el.mediaSection.style.display = 'none';
  ...
  }        // ==================== IMAGE UPLOAD HANDLERS ====================        function initUploadHandlers() {
             const uploadArea = document.getElementById('uploadArea');
             const fileInput = document.getElementById('imageUpload');
             const uploadPrompt = document.getElementById('uploadPrompt');
             const uploadPreview = document.getElementById('uploadPreview');
             const btnUploadImages = document.getElementById('btnUploadImages');
              // Click to browse         uploadArea.addEventListener('click',  () => {
                 fileInput.click();
               
    });
              // File selection         fileInput.addEventListener('change',  (e) => {
                 handleFileSelect(e.target.files);
               
    });
              // Drag and drop         uploadArea.addEventListener('dragover',  (e) => {
                 e.preventDefault();
                 e.stopPropagation();
              ...
              // Upload button         btnUploadImages.addEventListener('click',  () => {
     ...
     // 10MB limit          const uploadPrompt = document.getElementById('uploadPrompt');
             const uploadPreview = document.getElementById('uploadPreview');
             const btnUploadImages = document.getElementById('btnUploadImages');
              // Filter for valid image files         const imageFiles = Array.from(files).filter(file => {
    ...
    }          // Add to selected files (avoid duplicates)         imageFiles.forEach(file => {
              ...
              // Update UI         uploadPrompt.style.display = selectedFiles.length > 0 ? 'none' :  'block';
              ...
              selectedFiles.forEach((file,  index) => {
                 const reader = new FileReader();
                  reader.onload = (e) => {
                     ...
                     previewItem.innerHTML = `           <img src="${e.target.result}" alt="${file.name}">           <div class="preview-name" title="${file.name}">${file.name}</div>           <button class="preview-remove" data-index="${index}" title="Remove">Remove</button>         `;
                      // Remove button handler             previewItem.querySelector('.preview-remove').addEventListener('click',  (evt) => {
                         evt.stopPropagation();
  ...
  }        // ==================== LOCAL FILE UPLOAD ====================        async function uploadImagesToGitHub() {
             // Maintained for backward compatibility;
     route to local upload         return uploadImagesLocal();
           
  }        async function uploadImagesLocal() {
             let chapterName = getActiveChapterName();
             if (!chapterName) {
                 showError('Enter a chapter name first.');
                 return;
               
    }         // Ensure we have a current chapter context         currentEditingChapter = chapterName;
             ...
             const btnUploadImages = document.getElementById('btnUploadImages');
             const chapterFolder = getChapterFolder(chapterName);
              uploadProgress.style.display = 'block';
             uploadProgress.innerHTML = 'Uploading...';
             btnUploadImages.disabled = true;
             btnUploadImages.textContent = 'Uploading...';
              try {
                 // Make sure chapter folder exists           await fetch('/api/create-chapter',  {
                     method:  'POST',              headers:  {
           'Content-Type':  'application/json' 
        },              body:  JSON.stringify({
           chapterFolder 
                  ...
                  const filesPayload = await Promise.all(             selectedFiles.map(async (file) => ({
                       name:  file.name,                data:  await readFileAsBase64(file)             
      }))           );
                  const response = await fetch('/api/upload-images',  {
                     method:  'POST',              headers:  {
           'Content-Type':  'application/json' 
        },              body:  JSON.stringify({
           chapterFolder,  files:  filesPayload 
        })           
      });
                  const result = await response.json();
                 if (!response.ok) {
                 ...
                 currentPages = sortPagesByFilename([...currentPages,  ...newPaths]);
                 renderPageList(currentPages);
                 markUnsaved();
                 clearSelectedFiles();
                  const errorCount = result.errors?.length || 0;
                 if (errorCount > 0) {
                     showError(`Uploaded ${newPaths.length} file(s), ${errorCount} failed.`);
  ...
  }        // ==================== GITHUB API - FILE DELETION ====================        async function getFileSha(filePath,  token) {
             const response = await fetch(           `https://api.github.com/repos/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/contents/${filePath}?ref=${GITHUB_CONFIG.branch}`,            {
                   headers:  {
             ...
             try {
                 // Step 1:  Get file SHA (required for deletion)           const sha = await getFileSha(filePath,  token);
                  // Step 2:  Delete the file           const response = await fetch(             `https://api.github.com/repos/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/contents/${filePath}`,              {
                       method:  'DELETE',                headers:  {
                           'Authorization':  `token ${token}`,                  'Content-Type':  'application/json',                  'Accept':  'application/vnd.github.v3+json'               
        },                body:  JSON.stringify({
                           message:  `Delete ${filePath} via admin panel`,                  sha:  sha,                  branch:  GITHUB_CONFIG.branch               
        })             
      }           );
                  if (!response.ok) {
                     const error = await response.json();
             ...
             return new Promise((resolve,  reject) => {
                 const reader = new FileReader();
                 reader.onload = () => {
                     // Remove data URL prefix (data: image/png;
        base64, )             const base64 = reader.result.split(',')[1];
                     resolve(base64);
  ...
  }        // ==================== PAGE RENUMBERING ====================        async function renumberPages() {
             if (!currentEditingChapter) {
                 showError('No chapter is currently being edited.');
                 return;
               
    }          if (currentPages.length === 0) {
                 showError('No pages to renumber.');
                 return;
               
    }          if (!confirm('Renumber the files on disk to match this order?')) {
                 return;
               
    }          const btnRenumber = el.btnRenumberPages;
             const originalText = btnRenumber ? btnRenumber.textContent :  'Renumbering';
             const chapterFolder = getChapterFolder(currentEditingChapter);
              try {
                 if (btnRenumber) {
           ...
           chapterFolder,  order:  currentPages 
        })           
      });
                  const result = await response.json();
                 if (!response.ok) {
                     throw new Error(result.error || 'Renumber failed');
                   
      }            currentPages = result.paths || [];
                 renderPageList(currentPages);
                 markUnsaved();
                 showSuccess(`Renumbered ${currentPages.length} file(s).`);
  ...
  }        function showRenumberConfirmation() {
             return renumberPages();
           
  }        function hideRenumberConfirmation() {
             if (el.confirmModal) {
             ...
             // Reuse existing error display or create alert         alert('ERROR: ' + message);
           
  }        function showSuccess(message) {
             // Reuse existing success display or create alert         alert('SUCCESS: ' + message);
           
  }        // ==================== MODAL MANAGEMENT ====================        function showModal() {
       ...
       behavior:  'smooth' 
             ...
             if (currentEditingChapter && !names.includes(currentEditingChapter)) {
                 names.push(currentEditingChapter);
         ...
         chapter:  '',  pages:  [],  index:  0 
             ...
             previewState.chapter = name;
             previewState.pages = sortPagesByFilename(getPreviewPages(name));
       ...
       pages 
    ...
    }          if (previewState.index >= pages.length) {
                 previewState.index = pages.length - 1;
               
    }          const src = pages[previewState.index];
             ...
             if (el.previewPageLabel) {
                 el.previewPageLabel.textContent = `Page ${previewState.index + 1} / ${pages.length}`;
               
    }          el.previewFrame.style.display = 'block';
             el.previewEmpty.style.display = 'none';
              if (el.previewPrev) el.previewPrev.disabled = previewState.index <= 0;
             if (el.previewNext) el.previewNext.disabled = previewState.index >= pages.length - 1;
  ...
  }        // ==================== GITHUB INTEGRATION ====================        function showSettingsModal() {
             el.settingsModal.classList.add('active');
             // Load existing token (partially masked)         const token = localStorage.getItem(GITHUB_TOKEN_KEY);
             if (token) {
    ...
    }          // Confirm publish         if (!confirm('Are you sure you want to publish these changes to GitHub? This will update the live website.')) {
                 return;
             ...
             const originalText = publishBtn.textContent;
              try {
                 ...
                 publishBtn.textContent = 'Publishing...';
                  // Prepare data           const dataToCommit = {
                     chapters:  chapters,              lastUpdated:  new Date().toISOString(),              publishedBy:  'Admin Panel'           
      };
                  // Get current file SHA (required for updates)           let sha = null;
                 try {
                     const currentFile = await fetch(               `https://api.github.com/repos/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/contents/${GITHUB_CONFIG.dataFile}?ref=${GITHUB_CONFIG.branch}`,                {
                           headers:  {
      ...
      }            // Commit the file           const content = btoa(unescape(encodeURIComponent(JSON.stringify(dataToCommit,  null,  2))));
       // Base64 encode with UTF-8 support            const response = await fetch(             `https://api.github.com/repos/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/contents/${GITHUB_CONFIG.dataFile}`,              {
                       method:  'PUT',                headers:  {
                           'Authorization':  `Bearer ${token}`,                  'Accept':  'application/vnd.github.v3+json',                  'Content-Type':  'application/json'               
        },                body:  JSON.stringify({
                           message:  `Update chapters via Admin Panel - ${new Date().toISOString()}`,                  content:  content,                  branch:  GITHUB_CONFIG.branch,                  ...(sha && {
             sha:  sha 
          }) // Include SHA only if updating existing file               
        })             
      }           );
                  if (!response.ok) {
                 ...
                 alert('Published successfully!\n\nGitHub Action is updating the site.\nCheck back in 1-2 minutes.\n\nView workflow status at:\nhttps://github.com/bwondercomics/battle-bros-reader/actions');
                  // Reset button after delay           setTimeout(() => {
  ...
  }        // ==================== UTILITIES ====================        function escapeHtml(text) {
             const div = document.createElement('div');
             div.textContent = text;
             return div.innerHTML;
           
  }        function sanitizeFolderFromName(name = '') {
             let slug = name.toLowerCase().replace(/[^a-z0-9]+/g,  '-').replace(/^-+|-+$/g,  '');
             if (!slug) slug = `chapter-${Date.now()}`;
             return `chapters/${slug}`;
           
  }        function inferFolderFromPages(name) {
  ...
  }        function ensureChapterFolder(name = '') {
           ...
           chapterFolder 
             ...
             try {
                 const response = await fetch(MEDIA_FILE,  {
         cache:  'no-cache' 
      });
                 if (!response.ok) throw new Error('Failed to load media.json');
                 const data = await response.json();
                 mediaItems = Array.isArray(data)             ? data.map(m => ({
                       id:  m.id || `media-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`,                path:  m.path || '',                tags:  Array.isArray(m.tags) ? m.tags :  parseTags(m.tags || '')             
      }))             :  [];
             ...
             try {
                 await saveToServer(MEDIA_FILE,  mediaItems);
             ...
             if (el.mediaSection) {
                 ...
                 el.mediaSection.scrollIntoView({
         behavior:  'smooth',  block:  'start' 
      });
             ...
             el.mediaList.innerHTML = '';
              const filtered = mediaItems.filter(item => {
                 if (!term) return true;
                 return (             item.path.toLowerCase().includes(term) ||             (item.tags || []).some(t => t.includes(term))           );
              ...
              if (!filtered.length) {
                 ...
                 div.innerHTML = `         <div class="chapter-info">           <div class="chapter-name">${escapeHtml(item.path)}</div>           <div class="chapter-meta" style="opacity:0.8;">${escapeHtml(tagsText || 'No tags')}</div>         </div>         <div class="chapter-actions">           <button class="btn-small btn-edit" data-use="${escapeHtml(item.id)}">Use</button>         </div>       `;
           ...
           'Content-Type':  'application/json' 
                 ...
                 const existingMap = new Map(mediaItems.map(m => [m.path,  m]));
                 const postTagMap = new Map(             (posts || [])               .filter(p => p.image)               .map(p => [p.image,  Array.isArray(p.imageTags) ? p.imageTags :  parseTags(p.imageTags || '')])           );
                  let added = 0;
                 let updated = 0;
                  diskPaths.forEach(p => {
                     const existing = existingMap.get(p);
                     const postTags = postTagMap.get(p) || [];
                     if (existing) {
                         if ((!existing.tags || existing.tags.length === 0) && postTags.length) {
                             existing.tags = postTags;
                             updated += 1;
                           
          }             
        } else {
                         mediaItems.push({
                             id:  generateMediaId(),                  path:  p,                  tags:  postTags               
          });
                         added += 1;
                 ...
                 if (changed) {
                     ...
                     if (changed) {
                         setMediaStatus(`Synced ${added} new item(s)${updated ? `,  updated ${
            updated
          } tag set(s)` : ''}.`);
                       
        } else {
                         setMediaStatus('Media folder is already synced.');
             ...
             const existing = mediaItems.find(m => m.path === path);
             if (existing) {
                 ...
                 mediaItems.push({
                     id:  generateMediaId(),              path,              tags:  normalizedTags           
      });
         ...
         behavior:  'smooth',  block:  'start' 
  ...
  }        // ==================== EVENT HANDLERS ====================        function attachEventHandlers() {
             // Login         el.loginForm.addEventListener('submit',  async (e) => {
                 e.preventDefault();
              ...
              // Logout         el.btnLogout.addEventListener('click',  logout);
              // Add chapter         el.btnAddChapter.addEventListener('click',  addNewChapter);
             if (el.btnSaveStatus) {
                 el.btnSaveStatus.addEventListener('click',  saveStatusMessage);
               
    }          // Modal controls         el.btnCloseModal.addEventListener('click',  hideModal);
             el.btnCancelEdit.addEventListener('click',  hideModal);
             el.editForm.addEventListener('submit',  async (e) => {
                 e.preventDefault();
                 await saveChapterEdit();
               
    });
              // Page management         el.btnAddPage.addEventListener('click',  addPage);
              // Preview & export         el.btnPreview.addEventListener('click',  showPreview);
             ...
             if (el.btnMedia) {
                 ...
                 el.previewNext.addEventListener('click',  () => {
                     previewState.index = Math.min(               (previewState.pages.length || 1) - 1,                previewState.index + 1             );
                     renderPreviewImage();
                   
      });
               
    }          // GitHub integration         el.btnSettings.addEventListener('click',  showSettingsModal);
             ...
             el.btnPublish.addEventListener('click',  publishToGitHub);
              // Renumber pages         el.btnRenumberPages.addEventListener('click',  renumberPages);
             el.btnConfirmRenumber.addEventListener('click',  confirmRenumber);
             el.btnCancelRenumber.addEventListener('click',  hideRenumberConfirmation);
              // Close modals on backdrop click         el.editModal.addEventListener('click',  (e) => {
              ...
              el.confirmModal.addEventListener('click',  (e) => {
                 if (e.target === el.confirmModal) {
                     hideRenumberConfirmation();
                   
      }         
    });
           
  }        // ==================== INITIALIZATION ====================        async function init() {
             attachEventHandlers();
             initUploadHandlers();
  ...
  }        // ==================== START ====================        if (document.readyState === 'complete' || document.readyState === 'interactive') {
             setTimeout(init,  0);
